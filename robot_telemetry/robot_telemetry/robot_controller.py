...
# This node's job is to send out (publish) the position we want
# each joint to move to.
class RobotController(Node):
    def __init__(self):
        # Set up this node and give it a name.
        super().__init__('robot_controller')

        # Set up one publisher per joint, so we can send a number
        # for each joint on its own separate topic.
    ...
    def publish_commands(self):
        ...
        # 225 is outside Joint 2's allowed range on purpose,
        # so we can see the Monitor node catch it as unsafe.
        msg2 = Float64()
        msg2.data = 225.0
